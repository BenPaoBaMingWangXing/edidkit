...
`edidkit` parses untrusted EDID text or byte arrays without file-system, network, process, or OS display driver access. The parser rejects short inputs and invalid headers, reports checksum mismatch as a diagnostic, and decodes only the first 128-byte base block in version `0.1.0`.
